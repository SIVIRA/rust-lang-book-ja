...
このコードは`s`を`tic-tac-toe`に設定します。`format!`マクロは`println!`と同じように動作しますが、出力を画面に出力するのではなく、その内容を含む`String`を返します。`format!`を使ったコードのバージョンははるかに読みやすく、そのパラメータの所有権を取ることはありません。
